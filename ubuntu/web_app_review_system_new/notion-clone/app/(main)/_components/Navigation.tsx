...
import { createTask } from "@/src/mockBackend/mockApi";
...
const Navigation = () => {
  ...
  const handleCreate = async () => {
    const newTask = {
      title: "Untitled",
      content: "",
      role: "Programmer" as "Programmer" | "Reviewer",
      files: [],
      codeChanges: "",
      status: "pending" as "pending" | "approved" | "disapproved",
      isArchived: false,
      isPublished: false,
      createdBy: "user_id", // Replace with actual user ID when available
    };

    try {
      const documentId = await createTask(newTask);
